...
// Attribution-aware aggregate over the spend log — the input to a space's recap.
// Spend is stored per virtual KEY, which answers "who paid" but not "for what". A
// recap needs the second question, so calls carry a space/thread tag (set by the
// harness as a header) and this rolls them up over a time window.
//
// Note what is NOT here: files touched. The gateway sees model calls, not tool
// calls — claiming otherwise would be inventing data. Repeat detection is real
// though, and it's the more useful signal anyway.
export function readActivity({ space, thread, since = 0, until = Infinity } = {}) {
  const rows = readLogs(1e6).filter((r) => {
    const t = Date.parse(r.ts);
    if (!(t >= since && t <= until)) return false;
    if (space && r.space !== space) return false;
    if (thread && r.thread !== thread) return false;
    return true;
  });

  // A call the repeat brake refused never reached a model, so it is not a run and
  // must not land in the model/agent buckets or the token totals. It is still a
  // row, because what the brake STOPPED is the number that shows it earned its
  // keep — see `brakeTrips` below.
  const ran = rows.filter((r) => r.status !== 'declined');
  ...
  const bucket = (field) => {
    ...
    for (const r of ran) {
      const k = r[field];
      if (!k) continue;
      const b = m.get(k) || { [field]: k, runs: 0, cost: 0 };
      b.runs++; b.cost += r.spend || 0;
      m.set(k, b);
    }
    return [...m.values()].sort((a, b) => b.cost - a.cost || b.runs - a.runs);
  };

  // How much of this window was the same question asked again. Grouped on a hash
  // of the prompt — the log never stores prompt text (see promptSig in server.mjs).
  // Counted over calls that RAN, so it pairs cleanly with `declined`: redundant
  // work that happened, next to redundant work that was stopped.
  const sigs = new Map();
  for (const r of ran) if (r.prompt_sig) sigs.set(r.prompt_sig, (sigs.get(r.prompt_sig) || 0) + 1);
  const groups = [...sigs.values()].filter((n) => n > 1);
  const repeats = {
    inGroups: groups.reduce((s, n) => s + n, 0),         // calls sharing a question with another call
    redundant: groups.reduce((s, n) => s + n - 1, 0),    // ...minus one original per question
    largestGroup: groups.length ? Math.max(...groups) : 0,
  };

  return {
    space: space || null, thread: thread || null,
    since, until: until === Infinity ? null : until,
    runs: ran.length,
    cost: ran.reduce((s, r) => s + (r.spend || 0), 0),
    errors: ran.filter((r) => r.status === 'error').length,
    // QUESTIONS the brake stopped looping, not calls refused: the log holds one
    // row per trip so an ignored 429 can't flood it, which makes this a count of
    // runaway questions. That's the right grain for a recap anyway ("a loop was
    // stopped on 1 question" beats "412 calls refused"). For live per-refusal
    // counts during an incident, use GET /spend/brake.
    brakeTrips: rows.length - ran.length,
    tokens: {
      in: ran.reduce((s, r) => s + (r.prompt_tokens || 0), 0),
      out: ran.reduce((s, r) => s + (r.completion_tokens || 0), 0),
    },
    models: bucket('model'),
    agents: bucket('agent'),
    repeats,
