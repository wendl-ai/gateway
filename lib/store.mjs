...
export function readActivity({ space, thread, since = 0, until = Infinity } = {}) {
  ...
  const sigs = new Map();
  for (const r of rows) if (r.prompt_sig) sigs.set(r.prompt_sig, (sigs.get(r.prompt_sig) || 0) + 1);
  ...
  return {
    ...
    runs: rows.length,
    cost: rows.reduce((s, r) => s + (r.spend || 0), 0),
    errors: rows.filter((r) => r.status === 'error').length,
    tokens: {
      in: rows.reduce((s, r) => s + (r.prompt_tokens || 0), 0),
      out: rows.reduce((s, r) => s + (r.completion_tokens || 0), 0),
    },
    models: bucket('model'),
    agents: bucket('agent'),
    repeats,
