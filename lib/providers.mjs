...
// beta: the client's own `anthropic-beta` header, forwarded as-is on the native
// passthrough path. Without this, a client body field gated behind a beta flag
// (e.g. the Agent SDK's `context_management`) reaches Anthropic with no opt-in
// header and gets rejected — "lossless passthrough" has to mean headers too.
const anthHeaders = (route, beta) => ({
...
async function anthropicChat(route, body, signal, beta) {
  const r = await fetch(`${route.base}/v1/messages`, {
    method: 'POST', headers: anthHeaders(route, beta), signal, body: JSON.stringify(toAnthropic(body, route.upstreamModel)),
  });
...
async function anthropicStream(route, body, res, signal, beta) {
  const { status, json, usage } = await anthropicChat(route, body, signal, beta);
...
export function chat(route, body, signal, beta) {
  return route.format === 'anthropic' ? anthropicChat(route, body, signal, beta) : openaiChat(route, body, signal);
}
export function chatStream(route, body, res, signal, beta) {
  return route.format === 'anthropic' ? anthropicStream(route, body, res, signal, beta) : openaiStream(route, body, res, signal);
}
...
async function anthropicMessagesPassthrough(route, body, signal, beta) {
  const r = await fetch(`${route.base}/v1/messages`, {
    method: 'POST', headers: anthHeaders(route, beta), signal, body: JSON.stringify({ ...body, model: route.upstreamModel, stream: false }),
  });
  const json = await r.json();
...
async function anthropicMessagesStreamPassthrough(route, body, res, signal, beta) {
  const upstream = await fetch(`${route.base}/v1/messages`, {
    method: 'POST', headers: anthHeaders(route, beta), signal, body: JSON.stringify({ ...body, model: route.upstreamModel, stream: true }),
  });
  if (!upstream.ok || !upstream.body) {
    const text = await upstream.text().catch(() => '');
...
export function messages(route, body, signal, beta) {
  return route.format === 'anthropic' ? anthropicMessagesPassthrough(route, body, signal, beta) : openaiAsMessages(route, body, signal);
}
export function messagesStream(route, body, res, signal, beta) {
  return route.format === 'anthropic' ? anthropicMessagesStreamPassthrough(route, body, res, signal, beta) : openaiAsMessagesStream(route, body, res, signal);
}
